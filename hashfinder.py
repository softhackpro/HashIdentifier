...
algorithms={"102020":"ADLER-32", "102040":"CRC-32", "102060":"CRC-32B",
"101020":"CRC-16", "101040":"CRC-16-CCITT", "104020":"DES(Unix)",
"101060":"FCS-16", "103040":"GHash-32-3", "103020":"GHash-32-5", 
"115060":"GOSTR 34.11-94", "109100":"Haval-160", "109200":"Haval-160(HMAC)",
"110040":"Haval-192", "110080":"Haval-192(HMAC)", "114040":"Haval-224",
"114080":"Haval-224(HMAC)", "115040":"Haval-256",
"115140":"Haval-256(HMAC)","107080":"Lineage IIC4", 
"106025":"Domain CachedCredentials -MD4(MD4(($pass)).(strtolower($username)))",
...
"109040":"MySQL5 - SHA-1(SHA-1($pass))", "109180":"RipeMD-160(HMAC)", "109120":"RipeMD-160",
"109020":"SHA-1", "109140":"SHA-1(HMAC)", "109220":"SHA-1(MaNGOS)",
"109240":"SHA-1(MaNGOS2)", "109080":"Tiger-160", "109160":"Tiger-160(HMAC)",
"109260":"sha1($pass.$salt)", "109280":"sha1($salt.$pass)",
...
def delete():
	entry.delete(0, 'end')
	output_label1.delete(0, 'end')
	output_label2.delete(0, 'end')
	jerar.clear()
...
def find():
	...
	if len(jerar) == 0:
		...
		jerar.sort()
		output_label1.insert(END, str(algorithms[jerar[0]]))
		output_label1.insert(END, str(algorithms[jerar[1]]))

		for a in range(int(len(jerar))-2):
			output_label2.insert(END, str(algorithms[jerar[a+2]]))
	else:
		jerar.sort()
		for a in range(len(jerar)):
			output_label1(END, str(algorithms[jerar[a]]))


root = Tk()
root.title("Hash Identifier")
root.geometry("880x550+10+10")
root.resizable(False, False) 
root.configure(background="#d9d9d9")

frame1 = Frame(root)
h1 = Scrollbar(frame1, orient = 'horizontal')  
h1.pack(side = BOTTOM, fill = X)  
v1 = Scrollbar(frame1)  
v1.pack(side = RIGHT, fill = Y)

frame2 = Frame()
h2 = Scrollbar(frame2, orient = 'horizontal')  
h2.pack(side = BOTTOM, fill = X)  
v2 = Scrollbar(frame2)  
v2.pack(side = RIGHT, fill = Y)


message_label = Label(text="Enter a Hash", font=('Verdana', 16, 'bold'),bg='green', fg='white')
path = Label(text='Possible Hashs ', font=('Verdana', 17, 'bold'),bg='red', fg='white')
path1 = Label(text='Least Possible Hashs ', font=('Verdana', 17, 'bold'),bg='red', fg='white')
output_label1 = Listbox(frame1, xscrollcommand = h1.set, yscrollcommand = v1.set, bd=3, font=('Verdana', 12), bg='white', fg='#FFBC58', width=40, height=13)
output_label2 = Listbox(frame2, xscrollcommand = h2.set, yscrollcommand = v2.set, bd=3, font=('Verdana', 12), bg='white', fg='#72CE18', width=40, height=13)
entry = Entry(bd=2, font=('Verdana',12, 'bold'), bg='white', fg='red', width=55)
check_button = Button(bd=3, text="Find", font=('Verdana', 16), bg='red', fg='white', command=find)
clear_button = Button(bd=3, text="Clear", font=('Verdana', 16), bg='red', fg='white', command=delete)


message_label.place(x=20, y=50)
entry.place(x=200, y=50)
check_button.place(x=430, y=120)
clear_button.place(x=320, y=120)
path.place(x=30, y=200)
path1.place(x=460, y=200)
frame1.place(x=10,y=250)
output_label1.pack(side=BOTTOM)
frame2.place(x=445, y=250)
output_label2.pack()
h1.config(command=output_label1.xview)  
v1.config(command=output_label1.yview)
h2.config(command=output_label2.xview)  
v2.config(command=output_label2.yview)
